...
async function buildExcelBuffer(pickupList, day, area, isCombined, totalDropoffs, totalPickups) {
  ...
  for (const p of pickupList) {
    ...
    row.eachCell((cell, colNumber) => {
      ...
      if (isCombined && colNumber === 4) {
        cell.font = isPickup ? { ...greenFont, size: 12 } : { ...redFont, size: 12 };
      }
    });
  }
  ...
  if (isCombined) ws.getColumn(4).width = 14;

  ws.eachRow((row) => { row.height = 22; });
...
// GET: generate from server data (original behavior)
export async function GET(request) {
  ...
  try {
    ...
    let pickupList, isCombined = false, totalDropoffs = 0, totalPickups = 0;

    if (isDay2) {
      isCombined = true;
      const day1Emails = responses.filter((r) => r[3]?.toLowerCase() === config.day1.toLowerCase()).map((r) => r[2]?.toLowerCase());
      const day2Emails = responses.filter((r) => r[3]?.toLowerCase() === config.day2.toLowerCase()).map((r) => r[2]?.toLowerCase());
      const day1Customers = customers.filter((c) => c.emails.some((e) => day1Emails.includes(e.toLowerCase())));
      const day2Customers = customers.filter((c) => c.emails.some((e) => day2Emails.includes(e.toLowerCase())));
      ...
      const confirmedEmails = responses.filter((r) => r[3]?.toLowerCase() === day.toLowerCase()).map((r) => r[2]?.toLowerCase());
      const confirmedCustomers = customers.filter((c) => c.emails.some((e) => confirmedEmails.includes(e.toLowerCase())));
      totalPickups = confirmedCustomers.length;
      pickupList = buildPickupList(confirmedCustomers, keysMap, area, "pickup");
    }

    const { buffer, filename } = await buildExcelBuffer(pickupList, day, area, isCombined, totalDropoffs, totalPickups);
...
// POST: generate from user-reordered list sent from the dashboard
export async function POST(request) {
  try {
    const body = await request.json();
    const { pickupList, day, area, isCombined, totalDropoffs, totalPickups, pin } = body;

    if (pin !== process.env.ADMIN_PIN) return new Response("Unauthorized", { status: 401 });
    if (!pickupList || !day) return new Response("Missing data", { status: 400 });

    const { buffer, filename } = await buildExcelBuffer(pickupList, day, area, isCombined, totalDropoffs || 0, totalPickups || 0);
    return new Response(buffer, {
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="${filename}"`,
    ...
    console.error("XLSX POST error:", err);
    return new Response(JSON.stringify({ error: err.message }), { status: 500, headers: { "Content-Type": "application/json" } });
  }
}
