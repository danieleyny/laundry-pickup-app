...
// Live "driver is N min away" alert. Wording branches on the stop type so a
// drop-off customer isn't told to "put their bag out".
export function approachEmail({ etaWindowLabel, type = "pickup" }) {
  const isDropoff = type === "dropoff";
  const subject = isDropoff ? "Your laundry delivery is coming up" : "Your pickup is coming up";
  const sub = isDropoff
    ? "Our driver is approaching your building to drop off your clean laundry."
    : "Our driver is approaching your stop. Please make sure your bag is outside if it isn't already.";
  const preheader = isDropoff
    ? `Driver about ${etaWindowLabel} away with your clean laundry.`
    : `Driver about ${etaWindowLabel} away. Make sure your bag is outside if it isn't already.`;
  const calloutHtml = isDropoff
    ? `<strong>No action needed</strong> — we'll have your clean laundry to you shortly.`
    : `<strong>If your bag is already out</strong> — perfect, no action needed.`;
  const text = isDropoff
    ? `Your laundry delivery is coming up\n\nOur driver is about ${etaWindowLabel} away with your clean laundry. No action needed on your end.\n\n— The Laundry Day Team\n(646) 705-0600 · laundrydaynyc@gmail.com\n\nUnsubscribe: {{UNSUBSCRIBE_LINK}}`
...
export async function fireApproachAlerts({ area, day, weekId }) {
  ...
  for (const stop of due) {
    ...
    for (const email of matchingCustomer.emails) {
      ...
      const built = approachEmail({ etaWindowLabel: label, type: stop.type });
      try {
        await sendEmail({
          to: email,
