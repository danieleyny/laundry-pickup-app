...
// Normalize an address for matching: lowercase, collapse whitespace
...
async function getPickupResponses(area, weekId) {
  ...
  // Try to read existing responses (unbounded range so rows past 500 aren't silently dropped)
  try {
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: SHEET_ID,
      range: `'${tabName}'!A:F`,
...
// Computed in New York time so evening confirmations don't roll into the
// wrong week (Vercel servers run in UTC, which is 4-5 hours ahead of ET).
...
function buildEntry(c, keysMap, type) {
  const keyInfo = keysMap[normalizeAddress(c.address)] || {};
...
function sortByRoute(list, area) {
  ...
  list.sort((a, b) => {
    ...
    if (is953Columbus(a.address) && is953Columbus(b.address)) return 0;
...
function addUptownAutos(list) {
  ...
  // Match loosely so "214 West 102nd St" in the sheet still counts as the
  // same building as the auto entry "214 West 102nd" (no duplicates).
  for (const auto of UPTOWN_AUTO_PICKUPS) {
    const autoNorm = normalizeAddress(auto.address);
    const already = list.some((x) => {
      const norm = normalizeAddress(x.address);
      return (
        norm === autoNorm ||
        norm.startsWith(autoNorm + " ") ||
        autoNorm.startsWith(norm + " ")
      );
    });
...
// ─── Driver photos ───────────────────────────────────────────────────────────
// Image bytes live in Vercel Blob; this tab holds the searchable metadata
// (one row per photo) so photos are auditable from the Google Sheet.
...
// Create the Photos tab with headers if it doesn't exist yet
async function ensurePhotosTab(sheets) {
  try {
    await sheets.spreadsheets.values.get({
      spreadsheetId: SHEET_ID,
      range: `'${PHOTOS_TAB}'!A1:A1`,
    });
  } catch (e) {
    try {
      await sheets.spreadsheets.batchUpdate({
        spreadsheetId: SHEET_ID,
        resource: {
          requests: [{ addSheet: { properties: { title: PHOTOS_TAB } } }],
        },
      });
      await sheets.spreadsheets.values.update({
        spreadsheetId: SHEET_ID,
        range: `'${PHOTOS_TAB}'!A1:K1`,
        valueInputOption: "RAW",
        resource: { values: [PHOTOS_HEADERS] },
      });
    } catch (createErr) {
      // Tab might already exist but be empty
    }
  }
}

// Log one driver photo's metadata
async function logPhoto({ timestamp, week, area, day, address, unit, type, status, url, pathname, note }) {
  const sheets = getSheets();
  await ensurePhotosTab(sheets);
  await sheets.spreadsheets.values.append({
    spreadsheetId: SHEET_ID,
    range: `'${PHOTOS_TAB}'!A:K`,
    valueInputOption: "RAW",
    resource: {
      values: [[timestamp, week, area, day, address, unit, type, status, url, pathname, note]],
    },
  });
}

// Read photo records, newest first, with optional filters
async function getPhotos({ week, area, day, address } = {}) {
  const sheets = getSheets();
  let rows = [];
  try {
    const res = await sheets.spreadsheets.values.get({
    ...
    return []; // tab doesn't exist yet — no photos
  }
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    if (week && week !== "all" && photo.week !== week) continue;
    if (area && photo.area !== area) continue;
    if (day && photo.day.toLowerCase() !== day.toLowerCase()) continue;
    if (address && !normalizeAddress(photo.address).includes(normalizeAddress(address))) continue;
    photos.push(photo);
  }
...
// Remove photo metadata rows older than the cutoff (ms epoch); returns count removed
async function deletePhotoRowsOlderThan(cutoffMs) {
  const sheets = getSheets();
  let rows = [];
  try {
    const res = await sheets.spreadsheets.values.get({
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    // Keep rows with unparseable timestamps rather than silently destroying them
